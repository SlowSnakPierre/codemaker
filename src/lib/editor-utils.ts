...
export function updateTabContent(
  ...
  return tabs.map(tab => {
    if (tab.id === tabId) {
      ...
      if (isExternalUpdate) {
        return { ...tab, content: newContent, modified: false };
      }
      // Otherwise, mark as modified only if content has changed
      return { ...tab, content: newContent, modified: tab.content !== newContent };
    }
    return tab;
  });
}
