...
2. **Types** : Importez les types depuis `renderer/src/core/types`
3. **Configuration** : Utilisez les constantes de `renderer/src/core/config/app-config.ts`
4. **Services** : Accédez aux fonctionnalités via les classes de service appropriées
5. **Utilitaires** : Les fonctions utilitaires sont disponibles via `renderer/src/utils`
